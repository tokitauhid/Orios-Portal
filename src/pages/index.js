...
import { NoticeBanner, CountdownTimer, FeatureCard } from "@site/src/components/Cards";
...
function getGlobalCountdowns(events, assignments, labReports, count = 3) {
  ...
  const mapped = [
    ...
    ...assignments
      .filter(
        (a) => a.status === "pending" && new Date(a.dueDate).getTime() > now,
      )
      .map((a) => ({
        id: `asgn-${a.id}`,
        title: a.title,
    ...
    ...labReports
      .filter(
        (l) => l.status === "pending" && new Date(l.dueDate).getTime() > now,
      )
      .map((l) => ({
        id: `lab-${l.id}`,
        title: l.title,
...
const features = [
  {
    title: "Notes",
    description:
      "Access subject-wise notes, links, docs, and resources all in one place.",
    to: "/notes",
    type: "notes",
  },
  {
    title: "Assignments",
    description:
      "Track all your assignments with due dates and submission status.",
    to: "/assignments",
    type: "assignments",
  },
  {
    title: "Lab Reports",
    description: "Manage lab experiment reports organized by subject.",
    to: "/lab-reports",
    type: "lab-reports",
  },
  {
    title: "Calendar",
    description:
      "Interactive calendar with events, routine, and important dates.",
    to: "/calendar",
    type: "calendar",
  },
  {
  ...
  {
    title: "File Sharing",
    description: "Securely share and download class materials and resources.",
    to: "/files",
    type: "files",
  },
];
    ...
    async function init() {
      try {
        ...
        try {
          ...
          console.warn("Auto-status skipped:", e);
        }
      } catch (err) {
            ...
            <section className={styles.section}>
              ...
              >
                ...
                <p className={styles.sectionDesc}>
                  Active countdowns for your next few events
                </p>
              </div>
              ...
              {todayClasses.length > 0 ? (
                <div className={styles.todayGrid}>
                  {todayClasses.map((cls, i) => (
                    <div
                      key={i}
                      className={`${styles.todayCard} ${styles[cls.type + "Card"]}`}
                    >
                      <span className={styles.todayTime}>
                        {formatTime(cls.time)}
                      </span>
                      <h4 className={styles.todaySubject}>{cls.subject}</h4>
                      <span className={styles.todayRoom}>
                        {cls.room} · {cls.teacher}
                      </span>
                      <span
                        className={`${styles.todayBadge} ${cls.type === "lab" ? styles.labBadge : ""}`}
                      >
