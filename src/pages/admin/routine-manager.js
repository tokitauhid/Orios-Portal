...
function EditableChip({ value, onSave, onRemove, children, className }) {
  ...
  return (
    <div className={`${styles.timeSlotChip} ${className || ""}`}>
      {children}
      <button className={styles.chipEdit} title="Edit" onClick={startEdit}>
        ✏️
      </button>
...
export default function AdminRoutine() {
  ...
  if (!routine)
    return (
      <Layout title="Routine &amp; Subjects — Admin">
  ...
  const handleCellClick = (day, idx) => {
    ...
    setForm(
      slot
        ...
        : { subject: "", room: "", teacher: "", type: "lecture" },
    );
  };
  ...
  const handleSaveCell = () => {
    ...
    while (updated.schedule[editing.day].length <= editing.slotIdx)
      updated.schedule[editing.day].push(null);
    if (form.subject.trim()) {
      updated.schedule[editing.day][editing.slotIdx] = {
        time: routine.timeSlots[editing.slotIdx],
  ...
  const handleClearCell = () => {
    const updated = { ...routine, schedule: { ...routine.schedule } };
    if (updated.schedule[editing.day])
      updated.schedule[editing.day][editing.slotIdx] = null;
    setRoutine(updated);
    setEditing(null);
  };
  ...
  const renameTimeSlot = (oldTime, newTimeValue) => {
    ...
    Object.entries(routine.schedule).forEach(([day, slots]) => {
      updatedSchedule[day] = (slots || []).map((slot) => {
        if (slot && slot.time === oldTime)
          return { ...slot, time: newTimeValue };
        return slot;
      });
    });
    setRoutine((prev) => ({
  ...
  const handleAddSubject = async (e) => {
    ...
    if (subjects.includes(name)) {
      setSubError("Subject already exists.");
      return;
    }
    setSubError("");
  ...
  const handleRenameSubject = async (oldName, newName) => {
    if (!newName || subjects.includes(newName)) {
      setSubError(
        subjects.includes(newName)
          ? "That subject name already exists."
    ...
    Object.entries(routine.schedule).forEach(([day, slots]) => {
      updatedSchedule[day] = (slots || []).map((slot) => {
        if (slot && slot.subject === oldName)
          return { ...slot, subject: newName };
        return slot;
      });
    });
    ...
    try {
      ...
      setSubSuccess(
        `"${oldName}" renamed to "${newName}" and updated across all collections.`,
      );
    } catch {
      setSubSuccess(
        `"${oldName}" renamed to "${newName}" in subjects & routine. Other collections may need a manual refresh.`,
      );
    }
      ...
      <AdminLayout title="🗓️ Manage Routine &amp; Subjects">
        ...
        <section className={styles.sectionCard}>
          <div className={styles.sectionHeader}>
            ...
            <p className={styles.hint}>
              Subjects appear as filter options across Notes, Assignments, Lab
              Reports, and admin forms. Click ✏️ to rename a subject.
            </p>
          </div>
        ...
        <section className={styles.sectionCard}>
          ...
          <p className={styles.hint} style={{ marginBottom: "20px" }}>
            Click any cell to edit a subject slot. Click "Publish All" to
            persist all changes.
          </p>
                  ...
                  <tr key={day}>
                    ...
                    {routine.timeSlots.map((time, colIdx) => {
                      ...
                      return (
                        <td
                          key={time}
                          className={`${styles.cell} ${slot ? styles[slot.type] : ""}`}
          ...
          <div className={styles.editOverlay} onClick={() => setEditing(null)}>
            ...
            >
              <h3 className={styles.modalTitle}>
                Edit: {editing.day} —{" "}
                {formatTime(routine.timeSlots[editing.slotIdx])}
              </h3>
              ...
              <div className={styles.modalActions}>
                <button className={styles.clearBtn} onClick={handleClearCell}>
                  Clear Cell
                </button>
                <div style={{ flex: 1 }} />
                <button
                ...
                <button className={styles.addBtn} onClick={handleSaveCell}>
                  Save Cell
                </button>
              </div>
            </div>
...
function SubjectChip({ name, delay, onRename, onRemove, styles }) {
  ...
  if (editing) {
    return (
      <div
        className={`${styles.subjectCard} ${styles.subjectCardEditing}`}
      ...
      >
        ...
        />
        <button
          className={`${styles.removeSubBtn} ${styles.saveSubBtn}`}
          onMouseDown={(e) => {
            e.preventDefault();
            commit();
          }}
  ...
  return (
    ...
    >
      ...
      <button
        className={styles.editSubBtn}
        onClick={startEdit}
        title="Rename subject"
      >
        ✏️
      </button>
      <button className={styles.removeSubBtn} onClick={onRemove}>
        ✕
      </button>
    </div>
  );
