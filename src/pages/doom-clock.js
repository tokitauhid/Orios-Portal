...
export default function DoomClockPage() {
      ...
      try {
        ...
        const mapped = [
          ...
          ...asgns
            .filter(
              (a) =>
                a.status === "pending" && new Date(a.dueDate).getTime() > now,
            )
            .map((a) => ({
              id: `asgn-${a.id}`,
              title: a.title,
          ...
          ...labs
            .filter(
              (l) =>
                l.status === "pending" && new Date(l.dueDate).getTime() > now,
            )
            .map((l) => ({
              id: `lab-${l.id}`,
              title: l.title,
  ...
  return (
    <Layout
      title="Doom Clock — Orios Class"
      description="Global countdowns for all exams, assignments, and labs"
    >
      <div className={styles.page}>
        <header className={styles.header}>
          <h1 className={styles.title}>⏳ Doom Clock</h1>
          <p className={styles.subtitle}>
            Your active countdowns. Submit your work and finish your exams
